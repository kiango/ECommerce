    ...
    public class OrdersService : IOrdersService
    {
        ...
        public async Task<(bool IsSuccess, IEnumerable<Order> Orders, string ErrorMsg)> GetOrdersAsync(int customerId)
        {
            try
            {
                ...
                {
                    var content = await response.Content.ReadAsByteArrayAsync(); // get response content
                    var options = new JsonSerializerOptions() { PropertyNameCaseInsensitive = true };
                    var result = JsonSerializer.Deserialize<IEnumerable<Order>>(content, options); // deserialize the content
